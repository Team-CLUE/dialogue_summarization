...
if __name__ == '__main__':
    ...
    if args.mode == 'train' :
        ...
        '''
        fit_on_texts( # 주어진 text들에 맞게 vocabulary update
            texts )   

        texts_to_sequences( # 각 text를 integer sequnce로 변경
            texts )
        '''
        src_tokenizer = Tokenizer(num_words = src_vocab) # tokenizer = DistilBertTokenizer()
        ...
        text_max_len 길이로 padding, decoder는 summary_max_len으로 padding
        'pre'면 앞쪽에서 패팅, 'post'면 뒤쪽에서 패딩
        '''

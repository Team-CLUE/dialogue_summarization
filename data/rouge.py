...
# make install
# cd ~
# mecab -d /usr/local/lib/mecab/dic/mecab-ko-dic << 아무 작동 안하길래 걍 스킵함
# apt install curl -y
# apt install git -y
# bash <(curl -s https://raw.githubusercontent.com/konlpy/konlpy/master/scripts/mecab.sh) << 쉘스크립트로 어떻게 짜지??
...
class RougeScorer:
    ...
    def compute_rouge(self, ref_df, hyp_df):
        ...
        generated_summaries = hyp_df
        reference_summaries = ref_df
...
if __name__ == "__main__":
    ...
    ref = ["안녕하세요", "나는"]
    rouge = RougeScorer()
    print(rouge.compute_rouge(pred, ref))